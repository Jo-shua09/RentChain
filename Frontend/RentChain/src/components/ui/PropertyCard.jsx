...
export default function PropertyCard({ property }) {
  ...
  const handleClick = (property) => {
    navigate(`/dashboard/landlord-dashboard/properties/${property.title}`, {
      state: {
  ...
  return (
    <div className={`p-10 bg-white border border-gray-200 shadow-sm group rounded-xl hover:shadow-md`}>
      ...
      <h3 className="mb-2 text-4xl font-semibold">{property.title}</h3>
      ...
      {pathName === "/dashboard/tenant-dashboard/properties" ? (
        property.available ? (
          ...
          <p className="mt-10 text-3xl font-semibold text-center normal-case text-secondary">Property not available</p>
        )
      ) : (
        <ButtonTwo
          name="edit property"
